...
if __name__ == '__main__':
    ...
    multi_bias = TMultiGraph()
    multi_standard_dev = TMultiGraph()
    for nfreq in NFREQS:
      for npu in NPUS:
        for amplitude in AMPLITUDES:
          ...
          multi_standard_dev.Add(graph_standard_dev)
    ...
    multi_bias.Draw("APL")
    multi_bias.GetXaxis().SetTitle("n Samples")
    multi_bias.GetYaxis().SetTitle("(E - E_{true}) / E_{true}")
    leg.Draw()
    cc_bias.SaveAs("cc_bias.png")
 
    cc_standard_dev = TCanvas("cc_standard_dev","",800,600)
    multi_standard_dev.Draw("APL")
    multi_standard_dev.GetXaxis().SetTitle("n Samples")
    multi_standard_dev.GetYaxis().SetTitle("#sigma_{E} / E")
    leg.Draw()
